...
class ReportUserDidKycController extends GetxController {
  ...
  var reportUserDidKyc = ReportStaticUsersDidKyc().obs;

  ReportUserDidKycController({
    this.fromDateInput,
    this.toDateInput,
  }) {
  ...
  Future<void> getReportStaticPotential() async {
    ...
    try {
      var res =
      await RepositoryManager.reportRepository.getReportStaticUsersDidKyc(
        timeFrom: fromDay.value,
        timeTo: toDay.value,
      );

      reportUserDidKyc(res!.data!);
